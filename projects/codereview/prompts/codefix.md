...
DO NOT OVERTHINK:
- Use existing comments in the source to avoid over‑engineering. Respect explicit assumptions and design decisions. If a comment states an intentional trade‑off (e.g., "no need to check data type"), especially when marked 'VERIFIED!', then treat it as correct. You may add brief clarification comments for future reviewers.
- Assume inputs satisfy the function's contract and are validated by the caller. Do not add local validation unless the function violates its stated assumptions or crosses a security boundary.
